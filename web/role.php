...
if (isset($role))
{
  $res = sql_query("SELECT role_name FROM $tbl_users LIMIT 1");
  if (! $res)
  {
...
echo "<div id=\"role_form\">\n";
$sql = "SELECT id, role_name, disabled
          FROM $tbl_users";
$res = sql_query($sql);
$roles_defined = $res && (sql_count($res) > 0);
if (!$areas_defined)
{
  echo "<p>" . get_vocab("no roles") . "</p>\n";
}
else
{
  ...
  if ($n_displayable_roles == 0)
  {
    echo "<p>" . get_vocab("no roles enabled") . "</p>\n";
  }
  else
  {
    ...
    // The area selector
    echo "<label id=\"area_label\" for=\"Role_select\">" . get_vocab("Roles") . ":</label>\n";
    echo "<select class=\"room_area_select\" id=\"area_select\" name=\"role\" onchange=\"this.form.submit()\">";
    if ($is_admin)
    {
      if ($roles[0]['disabled'])
      {
    ...
    foreach ($roles as $a)
    {
      if ($is_admin || !$a['disabled'])
      {
        ...
        $selected = ($a['id'] == $roles) ? "selected=\"selected\"" : "";
        echo "<option $selected value=\"". $a['id']. "\">" . htmlspecialchars($a['role_name']) . "</option>";
      }
    }
    if ($is_admin)
    {
