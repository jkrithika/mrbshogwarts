...
function output_row(&$row)
{
    ...
    if (!in_array($key, $ignore_columns))
    {
      ...
      switch($key)
      {
        ...
        default:
          ...
          if (isset($select_options["users.$key"]) && //CHECK1
 ...
 if (isset($select_options["area.$key"]) && //CHECK1
              is_assoc($select_options["area.$key"]))
          {
            if (isset($select_options["area.$key"][$row[$key]]))
            {
              $col_value = $select_options["area.$key"][$row[$key]];
            }
            else
            {
              $col_value = '';
...
$areafield = sql_field_info($tbl_area);
...
if (isset($Action) && ( ($Action == "Edit") or ($Action == "Add") ))
{
  ...
  if ($Id >= 0) /* -1 for new users, or >=0 for existing ones */
  {
    ...
    //$result1 = sql_query("select area_name from $tbl_area where id = $Id");
    $data = sql_row_keyed($result, 0);
    //$data1 = sql_row_keyed($result1, 0);
    s//ql_free($result1);
    sql_free($result);
  }
  if (($Id == -1) || (!$data))
...
echo "<label id=\"area_label\" for=\"area_select\">" . get_vocab("Areas") . ":</label>\n";
