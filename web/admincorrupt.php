...
if (($is_areaadmin) ||($is_superadmin) ||  ($n_displayable_areas > 0))
{
  ...
  if (isset($area))
  {
    ...
    else
    {
      ...
      {
        $rooms[] = $row;
        if ($is_superadmin || $is_areaadmin || $is_roomadmin || !$row['disabled'])
        {
          $n_displayable_rooms++;
        }
      }
      ...
      else
      {
        ...
        foreach ($rooms as $r)
        {
          ...
          if ($is_superadmin || $is_areaadmin || $is_roomadmin || !$r['disabled'])
          {
  ...
  if (($is_superadmin || $is_areaadmin) && $areas_defined && !empty($area))
  {
  ?>
