...
$is_passwordviewer = (getCapability($user) == 22 );
...
$is_hacker = (getCapability($capability) == 'add entry');
...
else
{
  ...
  else
  {
    ...
    echo "<select class=\"room_area_select\" id=\"area_select\" name=\"area[]\" multiple = \"multiple\">";
...
if (isset($area))
{
  $res = sql_query("SELECT room_name, custom_html FROM $tbl_room WHERE id=$area LIMIT 1");
  if (! $res)
  {
    trigger_error(sql_error(), E_USER_WARNING);
    fatal_error(FALSE, get_vocab("fatal_db_error"));
...
$sql = "SELECT area_id, room_name, disabled
          FROM $tbl_room
...
$fields = sql_field_info($tbl_room);
  ...
  else
  {
    ...
    // The area selector
    echo "<label id=\"room_label\" for=\"room_select\">" . get_vocab("rooms") . ":</label>\n";
    echo "<select class=\"room_area_select\" id=\"room_select\" name=\"room[]\" multiple = \"multiple\">";
//onchange=\"this.form.submit()\    
if ($is_admin)
    {
      if ($rooms[0]['disabled'])
    ...
    foreach ($rooms as $r)
    {
      ...
      {
        if ($is_admin && !$done_change && $r['disabled'])
        {
          echo "</optgroup>\n";
          echo "<optgroup label=\"" . get_vocab("disabled") . "\">\n";
          $done_change = TRUE;
...
echo "</div>\n";
	...
	if($selected_option == "viewpassword,")	{
...
echo "<option $selected value=\"". $c['cid']. "\">" . htmlspecialchars($c['capabilities']) . "</option>";
	  ...
	  //if($is_hacker) {*/
	//if($values[$capability]=="viewpassword,")
//if(in_array("viewpassword,",$values[$capability]))

//if($is_passwordviewer)
...
//}    
          ...
          <input class="submit default_action" type="submit" value="<?php echo(get_vocab("View Selected")); ?>">
...
if (($Id >= 0) && ($level >= $min_user_editing_level) && ($level >= $data['level'])) 
      {
        ...
        echo "<input class=\"submit\" type=\"submit\" " . 
              ...
              "value=\"" . get_vocab("Delete this user") . "\">\n";
        echo "</div>\n";
        echo "</form>\n";
      }
...
if (isset($Action) && ($Action == "Update"))
{
  ...
  else
  {
	...
	echo "capability:". $_POST['capability'];
      ...
      switch ($fieldname)
      {
        ...
        case 'level':
          ...
          break;
        case 'Area Name': $q_string .= "&$fieldname=" . $values[$fieldname];
          if (!isset($values[$fieldname]))
          {
            $values[$fieldname] = 0;
          }
...
//	print "arraylength: ".count($res);
  ...
  if ($level >= $min_user_editing_level) /* Administrators get the right to add new users */
  {
    ...
    print "    <input type=\"hidden\" name=\"Id\" value=\"-1\">\n";//i think we can add the add_a_new_user code part here
...
//	print "arraylength: ".count($res);
    ...
    echo "<div id=\"user_list\" class=\"datatable_container\">\n";//and the code for table not to be displayed to be put here
