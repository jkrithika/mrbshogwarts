...
else
{
  ...
  for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
  {
    ...
    $params = array('label' => $row['area_name'],
                      'name'          => 'new_area',
                      'options'       => $area,
                      'force_assoc'   => TRUE,
                      'value'         => $row['area_name'],
  ...
  /*$params = array('label' => $row['area_name'],
    ...
    generate_checkbox($params);
  */
    //echo "<checkbox class=\"room_area_select\" id=\"area_select\" name=\"area\" multiple = \"multiple\">";
...
echo "<label id=\"room_label\" for=\"room_select\">" . get_vocab("Rooms") . ":</label>\n";
    echo "<select class=\"room_area_select\" id=\"room_select\" name=\"rooms\" multiple = \"multiple\"onchange=\"this.form.submit()\">";
//echo "</fieldset>\n";
echo "</form>\n";
*/
...
//$fields = sql_field_info($tbl_room);
  ...
  for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
  {
    ...
    $params = array('label' => $row['room_name'],
                      'name'          => 'new_area',
                      'options'       => $area,
                      'force_assoc'   => TRUE,
                      'value'         => $row['room_name'],
  ...
  else
  {
    ...
    //echo "<select class=\"room_area_select\" id=\"room_select\" name=\"rooms\" multiple = \"multiple\">";
//onchange=\"this.form.submit()\    
if ($is_admin)
    {
      if ($rooms[0]['disabled'])
    ...
    foreach ($rooms as $r)
    {
      if ($is_admin || !$r['disabled'])
      {
        ...
        $selected = ($a['id'] == $area) ? "selected=\"selected\"" : "";
        //echo "<option $selected value=\"". $a['id']. "\">" . htmlspecialchars($r['room_name']) . "</option>";
      }
    }
...
?>
          ...
          <input class="submit default_action" type="submit" value="Save Changes">"
...
if (isset($Action) && ($Action == "Update"))
{
  ...
  {
    Header("Location: edit_users_confirm.php");
    exit;
  }
